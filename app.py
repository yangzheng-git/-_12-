...
def current_abnormal():
    ...
    return events
def find_abnormal(date1:str,date2:str,key1:str,key2=None,if_order=False):#第三代数据获取demo，key1是大类，key2是小类，实现更加灵活多变的数据需求，iforder默认只取十个属性
    ...
    for data in datas:  # 只对给定日期的时间做统计
        if date1 <= data.CREATE_TIME[:10] and data.CREATE_TIME[:10]<=date2:
        #if date in  data.CREATE_TIME:
            if statistics.get(str(eval('data' + '.' + key1))+"的"+str(eval('data.'+key2))) == None :
                if key2!=None:#有两个关键字
                    #statistics[str(eval('data' + '.' + key1))+"+"+str(eval('data.'+key2))] = {}
                    statistics[str(eval('data' + '.' + key1))+"的"+str(eval('data.'+key2))] = 1
                else:#只有一个关键字
                    statistics[eval('data' + '.' + key1)] = 1
            else:
                if key2!=None:
                    if statistics[str(eval('data' + '.' + key1))+"的"+str(eval('data.'+key2))] == None:
                        statistics[str(eval('data' + '.' + key1))+"+"+str(eval('data.'+key2))] = 1
                    else:
                        statistics[str(eval('data' + '.' + key1))+"的"+str(eval('data.'+key2))] = int(
                            statistics[str(eval('data' + '.' + key1))+"的"+str(eval('data.'+key2))] + 1)
                else:
                    statistics[eval('data' + '.' + key1)] += 1
    ...
    if len(list1) != 0:
        if if_order == True:
            print(json.dumps(list1[:5],ensure_ascii=False))
            return json.dumps(list1[:5],ensure_ascii=False)
        else:
            print(json.dumps(list1,ensure_ascii=False))
            return json.dumps(list1,ensure_ascii=False)
    else:
...
def chart_1():
    ...
    data2=str(find_key2_by_key1('2018-' + time_month1 + '-' + time_day1,'2018-' + time_month2 + '-' + time_day2, 'OVERTIME_ARCHIVE_NUM','EVENT_TYPE_NAME')),
                           data3=find_abnormal('2018-10-25', '2018-10-29', 'COMMUNITY_NAME', 'SUB_TYPE_NAME'),
...
def chat_2():
    ...
    return render_template('街道事件.html',data=str(find_key2_by_key1('2018-' + time_month1+'-'+time_day1,'2018-' + time_month2+'-'+time_day2,'STREET_NAME','SUB_TYPE_NAME')),
                           data2=find_abnormal('2018-10-25', '2018-10-29', 'COMMUNITY_NAME', 'SUB_TYPE_NAME'),
                           M1=time_month1,D1=time_day1,M2=time_month2,D2=time_day2,current=current_user,abdata=abnormal(),dbnum=len(current_abnormal()))
#
#
#
...
def display():
    ...
    data=find_abnormal('2018-10-25','2018-10-29','COMMUNITY_NAME','SUB_TYPE_NAME')
    if current_user.first_login=='YES':
        current_user.first_login='NO'
        db.session.commit()
        if current_user.role=='管理员':
            print("hhhh")
            return render_template('display_manager.html', data=data,current=current_user, abdata=abnormal(), usernum=usernum, checknum=checknum, datanum=datanum,
                               dbnum=len(current_abnormal()),M1=time_month1,D1=time_day1,M2=time_month2,D2=time_day2)
        else:
            print("hhhh1")
            return render_template('display_user.html',data=data,current=current_user, abdata=abnormal(), usernum=usernum, checknum=checknum, datanum=datanum,
                               dbnum=len(current_abnormal()),M1=time_month1,D1=time_day1,M2=time_month2,D2=time_day2)
    else:
        print("hhhh2")
        return render_template('display.html', data=data,current=current_user, abdata=abnormal(), usernum=usernum, checknum=checknum, datanum=datanum,
                               dbnum=len(current_abnormal()),M1=time_month1,D1=time_day1,M2=time_month2,D2=time_day2)
